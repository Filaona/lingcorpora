    ...
    if not strong:
        ...
        for elem in strong:
            ...
            if len(left_list) > n_results:
                break
        if not kwic:
            temp = [left_list[i] + center_list[i] + right_list[i] for i in range(min(len(left_list), n_results))]
            s = [[corpus[i], temp[i]]
                 for i in range(len(temp))]
        else:
            s = [[corpus[i], left_list[i].strip(), center_list[i].strip(), right_list[i].strip()]
...
def write_results(query, results, cols):
    ...
    with open('eesti_search_' + str(query) + '.csv', 'w', encoding='iso8859-15') as f:
